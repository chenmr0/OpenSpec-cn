...
<p align="center">
  <a href="https://github.com/studyzy/OpenSDD/actions/workflows/ci.yml"><img alt="CI" src="https://github.com/studyzy/OpenSDD/actions/workflows/ci.yml/badge.svg" /></a>
  ...
  <a href="https://nodejs.org/"><img alt="node version" src="https://img.shields.io/node/v/@studyzy/opensdd?style=flat-square" /></a>
...
或者通过源码安装
```bash
git clone https://github.com/studyzy/OpenSDD.git
cd OpenSDD
make install
```
...
或者安装到您的配置文件：
```bash
nix profile install github:studyzy/OpenSDD
```
...
```nix
{
  inputs = {
    ...
    openspec.url = "github:studyzy/OpenSDD";
  };
...
- 运行 `opensdd list` 来验证设置并查看任何活跃的变更
- 如果您的编程助手没有立即显示新的斜杠命令，请重新启动它。斜杠命令在启动时加载，因此重新启动可确保它们出现

### 可选：填充项目上下文

`opensdd init` 完成后，您将收到一个建议提示，帮助填充项目上下文：
...
| 功能 | 英文版 | 中文版 |
|------|--------|--------|
| 包名 | `@fission-ai/openspec` | `@studyzy/opensdd` |
| 命令 | `openspec` | `opensdd` |
| 列出变更 | `openspec list` | `opensdd list` |
| 初始化 | `openspec init` | `opensdd init` |
| 验证 | `openspec validate` | `opensdd validate` |
...
```bash
$ opensdd list                             # 确认变更文件夹存在
$ opensdd validate add-profile-filters     # 验证规范格式
$ opensdd show add-profile-filters         # 审查提案、任务和规范增量
```
...
AI: 我将归档 add-profile-filters 变更。
    *运行: opensdd archive add-profile-filters --yes*
...
```bash
opensdd list               # 查看活跃的变更文件夹
opensdd view               # 规范和变更的交互式仪表板
opensdd show <变更名称>     # 显示变更详情（提案、任务、规范更新）
opensdd validate <变更名称> # 检查规范格式和结构
...
1. **升级包**
   ...
   ```
2. **刷新代理指令**
    - 在每个项目中运行 `opensdd update`，重新生成AI指导并确保最新的斜杠命令处于活动状态。
