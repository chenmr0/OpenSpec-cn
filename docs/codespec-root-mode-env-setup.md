...
> 面向：希望"在 AI 工具里一次性配置、拉代码后自动让 `CODESPEC_ROOT_MODE=cwd` 生效"的协作团队。
...
| 取值 | 行为 |
|---|---|
| `git`（默认，未设置或任意非 `cwd` 值） | 从启动目录逐级向上查找 `.git`（文件或目录均算，兼容 git worktree 指针），将其所在目录作为根目录；走到文件系统根仍未找到则回退启动目录 |
| `cwd` | 直接以启动目录作为根目录，不向上查找 `.git` |

- 默认 `git`：在 `src/components/` 等子目录执行时，根目录=最近的 `.git` 所在仓库根，适合 monorepo 以仓库为根的场景。
- 设为 `cwd`：在子目录执行时根目录=该子目录本身。
...
```ts
// codespec-env.ts —— 启动时把 CODESPEC_ROOT_MODE=cwd 持久化到用户环境
export const CodespecEnvPlugin = async () => {
  ...
  if (process.platform === "win32") {
    ...
    spawnSync("setx", ["CODESPEC_ROOT_MODE", "cwd"], { encoding: "utf8" });
  } else {
    ...
    const marker = "export CODESPEC_ROOT_MODE=cwd # codespec-env";
  ...
  return {
    ...
    "shell.env": async (_input, output) => {
      output.env.CODESPEC_ROOT_MODE = "cwd";
    },
  };
};
```
...
- 在含 `.opencode/plugins/codespec-env.ts` 的目录运行 `opencode run "..."`：
  ...
  - PowerShell 查询 `[Environment]::GetEnvironmentVariable('CODESPEC_ROOT_MODE','User')` 返回 `cwd`。
...
- 在 `.cac/extensions.json` 声明条目 + 放置扩展文件后，在该目录运行 `codeagentcli.exe`：
  ...
  - 扩展函数体执行，`setx` 退出码 0。
  - PowerShell 查询 User 级环境变量返回 `cwd`。
...
3. **用户全局，非项目隔离**：`setx` 写的是当前 Windows 用户的环境变量，**影响该用户所有项目**——别的项目里 `codespec` 也会变成 cwd 模式。撤销：`setx CODESPEC_ROOT_MODE ""` 或用 PowerShell `[Environment]::SetEnvironmentVariable('CODESPEC_ROOT_MODE', $null, 'User')`。
4. **首次可能弹信任**：opencode 无信任弹窗；CodeAgent 若改用 `.cac/settings.json` 的 `env` 字段会触发 `hasDangerousEnvVars` 白名单弹窗，但走上面的扩展 `shell.env` 可绕开。

> 这意味着：本方案本质是"用户级一次性开启"，不是真正的项目隔离。如果团队里每个人都希望 cwd 模式，这套 hook 让每人首次启动工具时自动 setx 一次，之后全局生效——这正是"拉代码者自动生效"的落点。若要项目隔离（不同项目不同根模式），应改用 `CODESPEC_ROOT_MODE` 的进程级注入（如 opencode `shell.env` 不带 setx），而非 setx。

---

## 六、单机自用的更简做法

如果你只是自己一台机器要用 cwd 模式，根本不需要 hook——手动执行一次即可一劳永逸：

```bash
# Windows（PowerShell 或 cmd）
setx CODESPEC_ROOT_MODE cwd
# 或 PowerShell
[Environment]::SetEnvironmentVariable('CODESPEC_ROOT_MODE','cwd','User')

# Unix（写入 shell 配置）
echo 'export CODESPEC_ROOT_MODE=cwd' >> ~/.bashrc
```

之后重开终端，所有 `codespec` 命令均走 cwd 模式。hook 方案的价值仅在于"让协作团队里每个拉代码的人首次启动 AI 工具时自动执行这一步"。
...
3. 在该工具会话里让 agent 跑一条 shell 命令检查当前会话注入：
   ...
   # 应输出 cwd（由 shell.env 兜底注入）
   ```
4. 在项目子目录跑 `codespec status`，确认根目录解析为启动目录而非 `.git` 所在目录：
   ```bash
   cd src/components && codespec status   # 根目录应为 src/components 而非仓库根
   ```
