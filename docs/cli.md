...
OpenSpec CLI（`opensdd`）提供一组终端命令，用于项目初始化、校验、状态查看与管理。这些命令与 AI 的斜杠命令（如 `/opsx:propose`）相互补充；斜杠命令详见 [命令](commands.md)。
...
# 获取特定值
opensdd config get telemetry.enabled

# 设置值
opensdd config set telemetry.enabled false
...
如果没有配置更改，但当前项目文件与全局配置文件/交付方式不同步，OpenSpec 会显示警告并建议运行 `opensdd update`。
