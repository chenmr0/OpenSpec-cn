...
本指南介绍在你完成安装与初始化之后，OpenSDD 是如何工作的。安装说明请参见 [主 README](../README.md#quick-start)。

## 工作原理

OpenSDD 帮助你和你的 AI 编程助手在编写任何代码之前，就要构建的内容达成一致。
...
默认全局配置文件为 `core`，包含 `propose`、`explore`、`apply` 和 `archive`。你可以通过 `opensdd config profile` 启用扩展工作流命令，然后运行 `opensdd update`。

## OpenSDD 创建的内容

在运行 `opensdd init` 后，你的项目会拥有如下结构：
...
增量规范是 OpenSDD 的核心概念。它们显示相对于当前规范正在变化的内容。
...
- [自定义](customization.md) - 让 OpenSDD 按你的方式工作
