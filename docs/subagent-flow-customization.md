...
| 模式 | 别名 | 定制入口 | 命令 |
|------|------|----------|------|
| **subagent 模式** | 质量优先 | `subagent-apply.taskFlow.flowDot`（本文） | `codespec apply-subagent flow` |
...
开箱即用时（未配置 `subagent-apply.taskFlow.flowDot`），每个任务走一条固定的审查链，全部任务完成后再做变更级验证：
...
`config.yaml` 中的 `subagent-apply.taskFlow.flowDot`、`/apply` 命令、`subagent-driven-development` 技能、以及被分派的 subagent 之间的关系：
...
flowchart TD
    ...
    Skill -->|"执行任务前第 1 步"| Cmd["运行命令<br/>codespec apply-subagent flow"]
    Cmd --> Read["读取 codespec/config.yaml"]
    Read --> Key{"subagent-apply.<br/>taskFlow.flowDot<br/>存在且为字符串?"}
    Key -- "是：用户自定义" --> UserDot["用户 dot 原文<br/>原样透传，不校验"]
    Key -- "否 / 缺省 / 非字符串" --> DefaultDot["内置默认 dot<br/>DEFAULT_SUBAGENT_FLOW_DOT"]
    UserDot --> Ret["返回 dot<br/>source: config / default"]
    DefaultDot --> Ret
    Ret --> Skill
...
- **`/apply`（subagent 模式）**加载 `subagent-driven-development` 技能，由它在分派任何子 agent 前第一步调 `codespec apply-subagent flow` 取回 dot 流程图，然后严格按图分派。
- **`config.yaml` 的 `subagent-apply.taskFlow.flowDot` 是定制点**：有值就原样透传用户 dot，无值回退内置默认。改这里即改变整张流程图。
...
来源：未配置 subagent-apply.taskFlow.flowDot，使用内置默认流程
...
```bash
codespec apply-subagent flow --change add-dark-mode --json
```

返回 `{ changeName, source, flowDot }`，其中 `source` 为 `"config"`（使用了你的配置）或 `"default"`（回退到内置默认）。

### 第 2 步：在 config.yaml 写入 dot

编辑 `codespec/config.yaml`，新增顶层键 `subagent-apply`（注意：是顶层独立段，**不要**写在既有的 `apply:` 下面）：
...
- `flowDot` 是**字符串**，多行用 YAML block scalar。推荐用 `|-`（strip 尾随换行），避免末尾多一个空行。
- 内容是**合法的 graphviz dot**即可，CodeSpec 不校验内容、不解析 agent 名。
- 缺省（整段不写、或 `flowDot` 留空、或值为非字符串）→ 命令回退到内置默认 dot 并告警。

> 提示：`codespec init` 生成的 `config.yaml` 里已经内置了一段**被注释的默认 dot 模板**，取消注释即得默认行为，可直接在其上修改，无需从零手写。
...
确认"来源"一行变为 `config.yaml subagent-apply.taskFlow.flowDot（用户自定义）`，且流程图是你写的内容。若仍显示"使用内置默认流程"，检查 YAML 缩进与 `|-` 是否正确。

---

## 定制示例

### 示例 1：跳过代码质量审查（仅保留规格审查）
...
### 示例 2：拆分为"业务实现 / 测试验证"两阶段

每个任务先走业务实现 + 业务审查，再走测试编写 + 测试审查：
...
`codespec-subagent-driven-development` 技能在执行任何任务前的**第一步**就是运行 `codespec apply-subagent flow`，读取返回的 dot，然后严格按图执行：
...
检查：①YAML 缩进是否正确（`flowDot` 在 `taskFlow` 下、`taskFlow` 在 `subagent-apply` 下）；②是否用了 `|-` 而非 `|`（不致命，但会多一个尾随换行）；③值是否是非字符串（如写成 `flowDot: 123`），此时会告警并回退默认。

**Q：dot 写错了会怎样？**
CodeSpec 不校验 dot 语法。语法错误会导致执行 agent 解析失败，责任在作者。建议先用 `codespec apply-subagent flow` 确认输出，再实际跑 `apply`。
