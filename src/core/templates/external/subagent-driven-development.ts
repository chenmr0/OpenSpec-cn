...
digraph process {
    ...
    subgraph cluster_per_task {
        ...
        "实现子智能体实现、测试、报告修改文件、自审" [shape=box];
    ...
    "实现子智能体有疑问?" -> "实现子智能体实现、测试、报告修改文件、自审" [label="否"];
    "实现子智能体实现、测试、报告修改文件、自审" -> "分派规格审查子智能体 (spec-reviewer)";
...
Task tool (general-purpose):
  ...
  prompt: |
    ...
    **现在就问。** 在开始工作之前提出任何疑虑。
    ...
    3. 验证实现是否正常工作
    4. 报告你修改/新增/删除的明确文件路径；不要执行 git commit，最终由 /codespec/apply 统一提交
    5. 自审（见下文）
    6. 汇报
