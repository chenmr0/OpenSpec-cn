...
**探索项目上下文**
    从用户的描述中提取核心关键词，尤其是业务概念、依赖符号、接口名、配置项、测试目录和可能的模块名。进行**最多 50 次**工具调用（Glob/Grep），充分定位相关代码。

   探索目标（按需，不是每项必做）：
   - 优先检查相关模块的\`DESIGN.md\` ，因为\`DESIGN.md\`通常包含全量实现设计
   - 查看当前项目状态（文件、文档、最近的 commit）
   - 找到与用户需求最直接相关的现有实现（通常 3-5 个参考文件即可）
   - 探索需求对应的 DT 目录，**完整阅读相关README.md**，务必找到相关用例文件路径及执行命令
...
### 编写  task.md（任务计划文档）
    ...
    a. **强制使用外部技能创建计划。你必须按顺序执行：**
       ...
       **第一步：展示任务拆分粒度并确认。** 根据 spec.md 和 design.md，列出计划拆分的任务清单（仅任务名称，如"任务 1：用户认证模块开发"），**推荐总任务数 3–6 个；小变更可以更少。**
       ...
       **第二步：使用 **AskUserQuestion tool** 询问用户，是否启用测试驱动开发（TDD）。** 如果用户选择启用，则每个任务必须先写失败测试，再写最少实现**。

       **第三步：加载 \`writing-plans\` 技能创建详细的实现计划。** 严格按照第一步确认的任务粒度输出 task.md。
