...
/**
 * Generates Bash completion scripts for the CodeSpec CLI.
...
export class BashGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    return `# Bash completion script for CodeSpec CLI
...
${helpers}
complete -F _codespec_completion codespec
`;
  }
  ...
  private generatePositionalCompletion(positionalType: string | undefined, indent: string): string[] {
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(`${indent}_codespec_complete_changes`);
        break;
      case 'spec-id':
        lines.push(`${indent}_codespec_complete_specs`);
        break;
      case 'change-or-spec-id':
        lines.push(`${indent}_codespec_complete_items`);
        break;
      case 'shell':
