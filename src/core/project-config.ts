...
export function readProjectConfig(projectRoot: string): ProjectConfig | null {
  ...
  let configPath = path.join(projectRoot, 'codespec', 'config.yaml');
  if (!existsSync(configPath)) {
    configPath = path.join(projectRoot, 'codespec', 'config.yml');
    if (!existsSync(configPath)) {
  ...
  try {
    ...
    if (!raw || typeof raw !== 'object') {
      console.warn(`codespec/config.yaml is not a valid YAML object`);
      return null;
    }
    ...
    console.warn(`Failed to parse codespec/config.yaml:`, error);
    return null;
  }
}
...
export function suggestSchemas(
  ...
  let message = `Schema '${invalidSchemaName}' not found in codespec/config.yaml\n\n`;
