...
export const VALIDATION_MESSAGES = {
  ...
  GUIDE_NO_DELTAS:
    '未找到增量。确保您的变更在specs/目录下有功能文件夹（例如specs/http-server/spec.md），其中包含使用增量标题（## 新增需求/修改需求/移除需求/重命名需求）的.md文件，并且每个需求至少包含一个"#### 场景:"块。提示：运行"codespec change show <change-id> --json --deltas-only"来检查解析的增量。',
