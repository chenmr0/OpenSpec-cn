/**
 * Per-session state management for the codespec continuation enforcer.
 * Simplified from oh-my-openagent: removed compaction guard tracking.
 */

import type { ContinuationProgressOptions, ContinuationProgressUpdate, SessionState, Todo } from "./types.js";
...
export function createSessionStateStore(): SessionStateStore {
  ...
  function shutdown(): void {
    if (pruneInterval !== undefined) {
      clearInterval(pruneInterval as unknown as number);
    }
    cancelAllCountdowns();
    sessions.clear();
  }
