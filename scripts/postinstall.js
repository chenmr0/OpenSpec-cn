 ...
 * `codespec completion install` explicitly. This script only
 * prints a one-line tip after npm install.
 *
 * The tip is suppressed when:
 * - CI=true environment variable is set
 * - CODESPEC_NO_COMPLETIONS=1 environment variable is set
...
function shouldSkipInstallation() {
  ...
  if (process.env.CODESPEC_NO_COMPLETIONS === '1') {
    return { skip: true, reason: 'CODESPEC_NO_COMPLETIONS=1 set' };
  }
...
async function main() {
  try {
    ...
    console.log(`\n提示：运行 'codespec completion install' 以安装 shell 补全`);
  } catch (error) {
