...
describe('UpdateCommand', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `codespec-test-${randomUUID()}`);
  ...
  describe('basic validation', () => {
    it('should throw error if codespec directory does not exist', async () => {
      ...
      await expect(updateCommand.execute(testDir)).rejects.toThrow(
        "未找到CodeSpec目录。请先运行 'codespec init'。"
      );
    });
...
`;
      ...
      expect(updatedSkill).toContain('name: codespec-explore');
    ...
    it('should update core profile skill files when tool is configured', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'codespec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
  ...
  describe('multi-tool support', () => {
    it('should update multiple configured tools', async () => {
      ...
      await fs.mkdir(path.join(claudeSkillsDir, 'codespec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'codespec-explore', 'SKILL.md'),
        'old'
      );

      // Set up Cursor
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'codespec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(cursorSkillsDir, 'codespec-explore', 'SKILL.md'),
        'old'
      );
      ...
      expect(claudeSkill).toContain('name: codespec-explore');
      ...
      expect(cursorSkill).toContain('name: codespec-explore');
    ...
    it('should update Qwen tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(qwenSkillsDir, 'codespec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(qwenSkillsDir, 'codespec-explore', 'SKILL.md'),
        'old'
      );
    ...
    it('should update Windsurf tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(windsurfSkillsDir, 'codespec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(windsurfSkillsDir, 'codespec-explore', 'SKILL.md'),
        'old'
      );
  ...
  describe('skill content validation', () => {
    ...
    it('should include proper instructions in skill files', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'codespec-apply-change'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'codespec-apply-change', 'SKILL.md'),
        'old'
      );
...
`
    ...
    it('should update all tools when force is used with mixed versions', async () => {
      ...
      const { version } = await import('../../package.json');
      const claudeSkillDir = path.join(testDir, '.claude', 'skills', 'codespec-explore');
...
`
      ...
      const cursorSkillDir = path.join(testDir, '.cursor', 'skills', 'codespec-explore');
...
`
  ...
  describe('version tracking', () => {
    ...
    it('should only update tools that need updating', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'codespec-explore', 'SKILL.md');
...
`;
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('升级到新版本 CodeSpec')
      );
...
`;
    ...
    it('should cleanup legacy slash command directories with --force', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('已删除 .claude/commands/codespec/')
      );
    ...
    it('should cleanup legacy codespec/AGENTS.md with --force', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('已删除 codespec/AGENTS.md')
      );
...
`;
  ...
  describe('legacy tool upgrade', () => {
    it('should upgrade legacy tools to new skills with --force', async () => {
      ...
      const skillFile = path.join(testDir, '.claude', 'skills', 'codespec-explore', 'SKILL.md');
    ...
    it('should upgrade multiple legacy tools with --force', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'codespec-explore', 'SKILL.md');
      const cursorSkillFile = path.join(testDir, '.cursor', 'skills', 'codespec-explore', 'SKILL.md');
    ...
    it('should upgrade only unconfigured legacy tools when mixed', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'codespec-explore'), { recursive: true });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'codespec-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should create only effective profile skills when upgrading legacy tools', async () => {
      ...
      const nonCoreSkill = path.join(skillsDir, 'codespec-new-change', 'SKILL.md');
    ...
    it('should not inject non-profile workflows when upgrading legacy tools', async () => {
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'codespec-propose', 'SKILL.md')
      )).toBe(false);
  ...
  describe('profile-aware updates', () => {
    it('should generate only profile workflows when custom profile is set', async () => {
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'codespec-new-change', 'SKILL.md')
      )).toBe(true);
...
`
    ...
    it('should remove workflows outside profile during update sync', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'codespec-new-change'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'codespec-new-change', 'SKILL.md'), 'old');
  ...
  describe('new tool detection', () => {
    it('should detect new tool directories not currently configured', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'codespec-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'codespec-explore', 'SKILL.md'), 'old');
    ...
    it('should consolidate multiple new tools into one message', async () => {
      ...
      expect(consolidatedCalls[0]).toContain("codespec init");
  ...
  describe('scanInstalledWorkflows', () => {
    it('should detect installed workflows across tools', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'codespec-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'codespec-explore', 'SKILL.md'), 'content');
      await fs.mkdir(path.join(claudeSkillsDir, 'codespec-apply-change'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'codespec-apply-change', 'SKILL.md'), 'content');
    ...
    it('should return union of workflows across multiple tools', async () => {
      ...
      // Cursor has apply
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'codespec-apply-change'), { recursive: true });
      await fs.writeFile(path.join(cursorSkillsDir, 'codespec-apply-change', 'SKILL.md'), 'content');
