...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		it('should generate valid PowerShell completion script with header', () => {
			...
			expect(script).toContain('# PowerShell completion script for CodeSpec CLI');
			expect(script).toContain('$codespecCompleter = {');
			expect(script).toContain('Register-ArgumentCompleter');
		});

		it('should register argument completer for codespec command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize CodeSpec',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName codespec');
			expect(script).toContain('-ScriptBlock $codespecCompleter');
		});
		...
		it('should handle positional arguments for change-id', () => {
			...
			expect(script).toContain('Get-CodeSpecChanges');
		});
		...
		it('should handle positional arguments for spec-id', () => {
			...
			expect(script).toContain('Get-CodeSpecSpecs');
		});
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('function Get-CodeSpecChanges');
			expect(script).toContain('codespec __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('function Get-CodeSpecSpecs');
			expect(script).toContain('codespec __complete specs 2>$null');
		});
