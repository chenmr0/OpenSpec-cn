...
vi.mock('../../src/core/completions/installers/zsh-installer.js', () => ({
  ZshInstaller: vi.fn().mockImplementation(() => ({
    install: vi.fn().mockResolvedValue({
      success: true,
      installedPath: '/home/user/.oh-my-zsh/completions/_codespec',
      isOhMyZsh: true,
    ...
    uninstall: vi.fn().mockResolvedValue({
      success: true,
      message: '已从 /home/user/.oh-my-zsh/completions/_codespec 移除补全脚本',
    }),
  })),
}));
...
describe('CompletionCommand', () => {
  ...
  describe('generate subcommand', () => {
    it('should generate Zsh completion script to stdout', async () => {
      ...
      expect(output).toContain('#compdef codespec');
      expect(output).toContain('_codespec() {');
    });
